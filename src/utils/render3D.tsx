...
export async function loadFbxAsync({asset, onAssetRequested}: any) {
...
export async function loadGLTFAsync({asset, onAssetRequested}: any) {
  const uri = await loadFileAsync({
    asset,
    ...
    if (texturesLength === 1) {
      ...
      obj.traverse(function (object: any) {
        if (object instanceof THREE.Mesh) {
          ...
          // );
          // console.log(object);
          const selected = textures?.find(
            (x: {name: string}) => x.name === object.name,
          );
